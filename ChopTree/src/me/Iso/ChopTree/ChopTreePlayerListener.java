...
public class ChopTreePlayerListener implements Listener {
    ...
    private static ChopTreeFiles files;

    public ChopTreePlayerListener (ChopTree instance) {
        plugin = instance;
        files = new ChopTreeFiles(plugin);
    }
    ...
    public void onPlayerCommandPreprocess (PlayerCommandPreprocessEvent event) {
        ...
        if (plugin.options.contains("cmd" + split[0].toLowerCase())) {		
            ...
            if (Array.getLength(split) > 1) {
                ...
                } else if (split[1].toLowerCase().contains("protect") || split[1].equalsIgnoreCase("p")) {
                    event.setCancelled(true);
                    event.setMessage("/_");
                ...
                } else if (split[1].toLowerCase().contains("chunk") || split[1].equalsIgnoreCase("c")) {
                    event.setCancelled(true);
                    event.setMessage("/_");
                ...
                } else if (split[1].toLowerCase().contains("reload") || split[1].equalsIgnoreCase("r")) {
    ...
    public boolean denyPermission (String string, Player player) {
        ...
        if (deny) player.sendMessage(ChatColor.RED + "You have no permission to perform this command.");
