...
public class ChopTreeFiles {
    ...
    public int getOptionIndex (String name) {
        if (name.equalsIgnoreCase("ActiveByDefault") || name.equalsIgnoreCase("Default") || name.equalsIgnoreCase("a")) return 0;
        if (name.equalsIgnoreCase("UseAnything") || name.equalsIgnoreCase("Anything") || name.equalsIgnoreCase("u")) return 1;
        if (name.equalsIgnoreCase("MoreDamageToTools") || name.equalsIgnoreCase("Damage") || name.equalsIgnoreCase("m")) return 2;
        if (name.equalsIgnoreCase("InterruptIfToolBreaks") || name.equalsIgnoreCase("Interrupt") || name.equalsIgnoreCase("i")) return 3;
        if (name.equalsIgnoreCase("LogsMoveDown") || name.equalsIgnoreCase("Down") || name.equalsIgnoreCase("l")) return 4;
        if (name.equalsIgnoreCase("OnlyTrees") || name.equalsIgnoreCase("trees") || name.equalsIgnoreCase("o")) return 5;
        if (name.equalsIgnoreCase("EnableOverride") || name.equalsIgnoreCase("override") || name.equalsIgnoreCase("e")) return 6;
        return -1;
    }
    ...
    public void listOption (int index, CommandSender sender) {
        if (index == -1) {
            for (int counter = 0; counter <= maxopt; counter++) {
                if (plugin.options.contains(getOptionName(counter))) {
                    sender.sendMessage(ChatColor.AQUA + getOptionReadable(counter, true));
                } else {
                    sender.sendMessage(ChatColor.AQUA + getOptionReadable(counter, false));
                }
            }
        } else {
            if (plugin.options.contains(getOptionName(index))) {
                sender.sendMessage(ChatColor.AQUA + getOptionReadable(index, true));
            } else {
                sender.sendMessage(ChatColor.DARK_AQUA + getOptionReadable(index, false));
            }
        }	
    }
    ...
    public void toggleOption (String name, String arg) {
        ...
        if (arg.toLowerCase().contains("true") || arg.toLowerCase().contains("on") || arg.toLowerCase().contains("enable")) {

            if (!plugin.options.contains(name)) {
                plugin.options.add(name);
            }

        } else if (arg.toLowerCase().contains("false") || arg.toLowerCase().contains("off") || arg.toLowerCase().contains("disable")) {
    ...
    public void putMaterial (String name) {
        ...
        if (material != null) {
            ...
            String message = "Could not match material \"" + name +"\".";
